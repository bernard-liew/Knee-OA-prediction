...
df$cycle <- NULL
```
# Check and remove collinearity

```{r}
cor_mat <- df [-c(1:10, length (df))] %>%
  map (as.numeric) %>%
  bind_cols() %>%
  cor()

rm_var <- caret::findCorrelation(cor_mat, cutoff = 0.7, names = TRUE)

df <- df[!grepl(paste0(rm_var, collapse = "|"), names(df))]
```
...
train_df$group <- factor (ifelse (train_df$group == "HOA", 1, 0))
test_df$group <- factor (ifelse (test_df$group == "HOA", 1, 0))
```
...
o <- train_df$group %>% as.character %>% as.numeric()
...
o <- test_df$group %>% as.character %>% as.numeric()
