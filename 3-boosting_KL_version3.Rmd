...
top_vars <- as.character (selected_vars[1])

# cumul. probability for average KL0 curve -> looks similar but not exactly the
# same as median curves
preds <- pred_for_all(mod, df, which, aggregate_fun = mean,
                      filter_data = function(x) x[df$kl_severity==0,])
...
# same now for KL2/3/4
preds <- pred_for_all(mod, df, which, aggregate_fun = mean,
                      filter_data = function(x) x[df$kl_severity==2,])
p2a <- preds %>%
  filter (feature %in% top_vars) %>%
  mutate (feature = str_remove(feature, "s_X")) %>%
  mutate (feature = factor (feature, levels = str_remove(top4_vars, "s_X")),
...
preds <- pred_for_all(mod, df, which, aggregate_fun = mean,
                      filter_data = function(x) x[df$kl_severity==3,])
p3a <- preds %>%
  filter (feature %in% top_vars) %>%
  mutate (feature = str_remove(feature, "s_X")) %>%
  mutate (feature = factor (feature, levels = str_remove(top4_vars, "s_X")),
...
preds <- pred_for_all(mod, df, which, aggregate_fun = mean,
                      filter_data = function(x) x[df$kl_severity==4,])
p4a <- preds %>%
  filter (feature %in% top_vars) %>%
  mutate (feature = str_remove(feature, "s_X")) %>%
  mutate (feature = factor (feature, levels = str_remove(top4_vars, "s_X")),
...
tiff ("../manuscript/fig5.tiff", units = "in", height = 5, width = 9, res = 100)
  cowplot::plot_grid(prow, legend, rel_widths = c(3, .4))
dev.off()
...
p1a <- preds %>%
  #filter (feature %in% top_vars) %>%
  mutate (feature = str_remove(feature, "s_X")) %>%
  mutate (feature = factor (feature, levels = varz_lvl ),
          class = factor (class, labels = c("KL0", "KL2", "KL3", "KL4"))) %>%
  ggplot(aes(x = cycle, y = value, color = class)) + 
  geom_line(linewidth = 1) + 
...
tiff ("../manuscript/figSM1.tiff", units = "in", height = 5, width = 9, res = 100)
  cowplot::plot_grid(prow, legend, rel_widths = c(3, .4))
dev.off()
...
```{r}
varz_lvl <- c("Ipsi_Ankle", "Ipsi_Knee", "Ipsi_Hip", "Ipsi_Spine",     
              "Ipsi_Neck", "Ipsi_Shoulder", "Ipsi_Elbow", "Ipsi_Wrist",
              "Contr_Ankle", "Contr_Knee", "Contr_Hip", "Contr_Spine",     
              "Contr_Neck", "Contr_Shoulder", "Contr_Elbow", "Contr_Wrist")
...
f1 <- f1 %>%
  map (~as.data.frame(.x)) %>%
  bind_rows (.id = "varz") %>%
  mutate (kl_severity = rep (df$kl_severity, 16)) %>%
  group_by(kl_severity, varz) %>%
  summarise_at(vars(t0:t100), mean) %>%
  pivot_longer(cols = starts_with("t"),
               names_to = "cycle",
               values_to = "angle")  %>%
  mutate (cycle = parse_number(cycle)) %>%
  mutate (varz = str_remove(varz, "s_X"),
          varz = factor (varz, levels = varz_lvl))

f1_plot <- ggplot (f1) +
  geom_line (aes (x = cycle, y = angle, color = kl_severity), linewidth = 1) +
  facet_wrap(~ varz, scales = "free", nrow = 4, ncol = 4) +
  scale_color_manual(values = c("black", "blue", "darkgreen", "red")) +
  labs (y = "Angles (°)",
        x = "Stride (%)",
        color = "KL scale") +
